import type { ViteMockOptions, MockMethod, Recordable, RespThisType, MethodType } from './types'
  ...
  const middleware: NextHandleFunction = async (req, res, next) => {
    ...
    if (matchRequest) {
      ...
      if (isFunction(rawResponse)) {
        ...
        let mockResponse = await response
        if (isFunction(response)) {
          mockResponse = await response.bind(self)(
            {
              url: req.url as any,
              body,
              query,
